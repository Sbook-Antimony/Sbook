...
class Chat(object):
    def __init__(
        self,
        pairs=(),
        reflections=substitutions.reflections,
        call=_function_call,
        api={},
        normalizer=substitutions.normal,
        ...
        if default_template is None:
            default_template = path.join(
                path.dirname(path.abspath(__file__)), "local", language + ".template"
            )
    ...
    def __normalize(self, text):
        ...
        :rtype: str
        """
        return self._normalizer_regex.sub(
            lambda mo: self._normalizer[mo.string[mo.start() : mo.end()].lower()], text
        )
    ...
    def __block_tags(self, text, pos, length, index):
        ...
        while pos[index][2] != "endblock":
            if pos[index][2] == "learn":
                within_block["learn"] = {}
                index = self.__group_tags(
                    text,
                    pos,
                    within_block["learn"],
                    (lambda i: pos[i][2] != "endlearn"),
                    length,
                    index + 1,
                )
                index -= 1
    ...
    def __group_tags(self, text, pos, groups, condition, length, index=0, name=""):
        ...
        while condition(index):
            if pos[index][2] == "block":
                ...
                raise SyntaxError(
                    self.__error_message("group, block, or response", pos[index][2])
                )
        if name in groups:
    ...
    def __process_learn(self, pairs):
        for topic in pairs:
            ...
            self._pairs[topic]["defaults"].extend(
                [(i, self._condition(i)) for i in pairs[topic].get("defaults", [])]
            )
            for pair in pairs[topic]["pairs"][::-1]:
                ...
                if type(learn) != dict:
                    raise TypeError(
                        "Invalid Type for learn expected dict got '%s'"
                        % type(learn).__name__
                    )
                if not client:
                    raise ValueError(
                        "Each block should contain at least 1 client regex"
                    )
                self._pairs[topic]["pairs"].insert(
                    0,
                    (
                        self.__build_pattern(client),
    ...
    def _set_within(self, group):
        for i in group:
            group[i]["child"] = (
                self._set_within(group[i]["child"]) if group[i]["child"] else []
            )
        index = list(group)
    ...
    def _inherit(self, start_end_pair, action):
        ...
        for i in range(len(start_end_pair)):
            ...
            for j in range(len(start_end_pair)):
                if (
                    start_end_pair[i][0] < start_end_pair[j][0]
                    and start_end_pair[i][1] > start_end_pair[j][1]
                ):
                    group[i].append(j)
    ...
    def _condition(self, response):
        ...
        new_pos = [
            ...
            if (not start) or response[start - 1] != "\\"
        ]
        i = 0
        start_end_pair = []
        actions = []
        while new_pos:
            ...
            if response[new_pos[i - 1][0]] in "{[":
                ...
                if b_n != e_n or not (
                    ...
                    or (
                        response[begin_tag[0]] == "["
                        and response[end_tag[1] - 1] == "]"
                    )
                ):
                ...
                if b_n == 2:
                    statement = re.findall(
                        r"^[\s\t]*(if|endif|elif|else|chat|low|up|cap|call|topic)[\s\t]+",
    ...
    def _compile_reflections(normal):
        ...
        return re.compile(
            r"\b({0})\b".format("|".join(map(re.escape, sorted_reflection))),
            re.IGNORECASE,
        )
    ...
    def __if_handler(self, i, condition, response, session_id):
        ...
        while check:
            con = self._check_and_evaluate_condition(
                response, condition[i]["child"], start, end, session_id=session_id
            )
            i += 1
        ...
        return (
            (
                self._check_and_evaluate_condition(
                    response,
    ...
    def __chat_handler(self, condition, response, session_id):
        ...
        response = self.respond(
            self.__handler(condition, response, "chat", session_id),
            session_id=session_id,
        )
        self.attr["substitute"] = substitute
        return response
    ...
    def __call_handler(self, condition, response, session_id):
        ...
        response = self.call.call(
            self.__handler(condition, response, "call", session_id),
            session_id=session_id,
        )
        self.attr["substitute"] = substitute
        return response
    ...
    def __topic_handler(self, condition, response, session_id):
        self.topic[session_id] = self.__handler(
            condition, response, "topic", session_id
        ).strip()
        return ""

    @staticmethod
    def __get_start_pos(start, response, exp):
        return start + re.compile(r"([\s\t]*" + exp + r"[\s\t]+)").search(
            response[start:]
        ).end(1)
    ...
    def __map_handler(self, condition, response, session_id):
        ...
        content = (
            self._check_and_evaluate_condition(
                response, condition["child"], start, end, session_id=session_id
            )
            .strip()
            .split(":")
        )
        ...
        return (
            self._memory[session_id][name]
            if not think and name in self._memory[session_id]
            else ""
        )
    ...
    def __api_request(self, url, method, **karg):
        try:
            ...
            raise RuntimeError(
                "Couldn't connect to server (unreachable). Check your network"
            )
        except KeyError:
    ...
    def __api_handler(self, api_name, method_name, data={}):
        ...
        if "auth" in self._api[api_name]:
            try:
                api_params["cookies"] = self.__api_request(
                    **self._api[api_name]["auth"]
                ).cookies
            except Exception:
                raise ValueError(
                    "In api.json 'auth' of '%s' is wrongly configured." % api_name
                )
        param = (
            "params"
            if self._api[api_name][method_name]["method"].upper().strip() == "GET"
            else "data"
        )
        try:
        ...
        response_text = (
            response.json() if api_type.upper().strip() == "JSON" else response.content
        )
    ...
    def __substitute_from_client_statement(
        self, match, prev_response, extra_symbol="", session_id="general"
    ):
        """
        ...
        for m in re.finditer(r"%" + extra_symbol + "[0-9]+", prev_response):
            ...
            try:
                final_response += self._quote(
                    self._substitute(match.group(num)), session_id
                )
            except IndexError:
                pass
            prev = end
        ...
        prev = 0
        for m in re.finditer(
            r"%" + extra_symbol + "([a-zA-Z_][a-zA-Z_0-9]*)([^a-zA-Z_0-9]|$)",
            prev_response,
        ):
            start = m.start(1)
            end = m.end(1)
    ...
    def _check_and_evaluate_condition(
        self,
        response,
        condition=[],
        start_index=0,
        ...
        while i < len(condition):
            pos = condition[i]["start"] - (
                1 if condition[i]["action"] in ("map", "eval") else 2
            )
            final_response += self._check_and_evaluate_condition(
                response[start_index:pos], session_id=session_id
            )
            try:
                self.attr[session_id]["_quote"] = False
                temp_response = self.__action_handlers[condition[i]["action"]](
                    condition[i], response, session_id
                )
                self.attr[session_id]["_quote"] = _quote
                ...
                if condition[i]["action"] == "if":
                    response_txt, i = self.__if_handler(
                        i, condition, response, session_id
                    )
                    final_response += response_txt
            start_index = condition[i]["end"] + (
                1 if condition[i]["action"] in ("map", "eval") else 2
            )
            i += 1
        final_response += self._check_and_evaluate_condition(
            response[start_index:end_index], session_id=session_id
        )
    ...
    def __intend_selection(self, text, previous_text, current_topic):
        for patterns, parents, response, learn in self._pairs[current_topic][
            "pairs"
        ]:  # check each pattern
            for pattern in patterns:
    ...
    def __response_on_topic(
        self, text, previous_text, text_correction, current_topic, session_id="general"
    ):
        match = self.__intend_selection(
            text, previous_text, current_topic
        ) or self.__intend_selection(text_correction, previous_text, current_topic)
        if match:
            ...
            if learn:
                self.__process_learn(
                    {
                        self._wildcards(
                            (topic, self._condition(topic)),
                            match,
                            parent_match,
                            session_id=session_id,
                        ): {
                            "pairs": [
                                self.__substitute_in_learn(
                                    pair, match, parent_match, session_id=session_id
                                )
                                for pair in learn[topic]["pairs"]
                            ],
                            "defaults": [
                                self._wildcards(
                                    (default, self._condition(default)),
                                    match,
                                    parent_match,
                                    session_id=session_id,
                                )
        ...
        if self._pairs[current_topic]["defaults"]:
            return self.__chose_and_process(
                self._pairs[current_topic]["defaults"],
                DummyMatch(text),
                None,
                session_id,
            )
    ...
    def respond(self, text, session_id="general"):
        ...
        while current_topic_order:
            try:
                return self.__response_on_topic(
                    text, previous_text, text_correction, current_topic, session_id
                )
            except ValueError:
                pass
    ...
    def __substitute_in_learn(self, pair, match, parent_match, session_id="general"):
        return tuple(
            (
                self.__substitute_in_learn(
                    i, match, parent_match, session_id=session_id
                )
    ...
    def save_template(self, filename):
        with open(filename, "w") as template:
            for topic_name, sub_topic in self.__get_topic_recursion(
                self._pairs
            ).items():
                self.__generate_and_write_template(
                    template, self._pairs, topic_name, sub_topic
                )

    def __generate_and_write_template(
        self, template, pairs, topic, sub_topics, base_path=None, padding=""
    ):
        ...
        for patterns, parents, response, learn in pairs[full_path]["pairs"]:
            ...
            for parent in parents:
                template.write(
                    new_padding + "\t{% prev %}" + parent.pattern + "{% endprev %}\n"
                )
            for pattern in patterns:
                template.write(
                    new_padding
            ...
            for res in response:
                template.write(
                    new_padding + "\t{% response %}" + res[0] + "{% response %}\n"
                )
            if learn:
        ...
        while input_sentence != quit:
            ...
            if input_sentence:
                ...
                self.conversation[session_id].append(
                    self.respond(input_sentence, session_id=session_id)
                )
